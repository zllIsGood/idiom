...
class WordModel extends BaseClass {
    ...
    public getWordGroup(type): any[] {
        ...
        let obj = {} as any
        for (let i in book) {
            let str = book[i].wordStock.spell as string
            let s = str[0]
            if (!obj[s]) {
                obj[s] = []
            }
    ...
    private async query(word: string) {
        let res = await RequestUtil.getPromise({
            ...
            data: {
                word: encodeURI(word) // word
            }
        });
        console.log("STAGE_WORD data:", res);
        if (res.code === 0) {
            this.words[res.data.word] = res.data
            this.wordsSource[res.data.word] = res.data.source
            if (res.data.hasAdd) {
                this.hasAddBook[res.data.word] = true
            }
        }
    }
