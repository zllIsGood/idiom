 ...
 * @Last Modified time: 2020-09-04 18:31:21
 */
class HomeWin extends BaseEuiView {
    ...
    public open(...param: any[]): void {
        ...
        this.addTouchEvent(this.dailyBtn, this.onClick);
    ...
    private setHero() {
        ...
        x = data1[0] && data1[0].data2.w > 200 ? (data1[0].data2.w - 200) / 2 : 0
    ...
    public showRoleUpMC() {
        let colors = [0x09f7ce, 0x66f907, 0xf9f9f2]
        MathUtils.randomSortArray(colors)
        for (let i = 0; i < 3; i++) {
            let item = new HomeMC()
            item.drawColor = colors[i]
            item.xyData = {
                sx: 375 + 100 * Math.random() - 50,
                sy: 1050,
                tx: 375 + 150 * i + 150 * Math.random() - 225,
                ty: 350
            }
            this.addChild(item)
        }
    ...
    private onClick(e: egret.TouchEvent): void {
        switch (e.currentTarget) {
            ...
            case this.dailyBtn:
                //每日断案
                ViewManager.ins().open(DailyTaskWin)
                break;
        }
    }
